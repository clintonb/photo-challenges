...
describe Helpers do
  describe Twitter do
    describe '#tweet_contains_hashtags?' do
      ...
      subject(:tweet) { Twitter::Tweet.new(:id => 28_669_546_014, :entities => {:hashtags => hashtags_array}) }

      it 'returns false if hashtag is blank' do
        expect(Helpers::Twitter::tweet_contains_hashtags?(tweet, nil)).to be_false
        expect(Helpers::Twitter::tweet_contains_hashtags?(tweet, '')).to be_false
        expect(Helpers::Twitter::tweet_contains_hashtags?(tweet, ' ')).to be_false
      end

      it 'returns false if the tweet does not contain the given hashtag' do
        expect(Helpers::Twitter::tweet_contains_hashtags?(tweet, 'xxxxxxxx')).to be_false
      end

      it 'returns true if the tweet contains the hashtag' do
        expect(Helpers::Twitter::tweet_contains_hashtags?(tweet, 'twitter')).to be_true
      end

      it 'is case-insensitive' do
        expect(Helpers::Twitter::tweet_contains_hashtags?(tweet, 'tWitTeR')).to be_true
        expect(Helpers::Twitter::tweet_contains_hashtags?(tweet, 'FaCEBooK')).to be_true
      end

      it 'supports finding multiple hashtags' do
        expect(Helpers::Twitter::tweet_contains_hashtags?(tweet, 'twitter', 'facebook')).to be_true
      end
    end

    describe '#tweet_image_url' do

      subject(:url) { Helpers::Twitter::tweet_image_url(Twitter::Tweet.new(:id => 28_669_546_014, :entities => entities)) }
    ...
    describe '#get_hashtags' do
      subject(:tweet) { Twitter::Tweet.new(:id => 28_669_546_014, :entities => {:hashtags => [{:text => 'Twitter', :indices => [10, 33]}, {:text => 'facebook', :indices => [0, 8]}]}) }
      it 'returns a lowercase array of hashtags in the tweet' do
        expect(Helpers::Twitter::get_hashtags(tweet)).to match_array %w(twitter facebook)
      end
    end
    ...
    describe '#process_tweet' do
      ...
      subject(:photo) { Helpers::Twitter::process_tweet(tweet) }
